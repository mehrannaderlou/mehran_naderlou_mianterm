...
<style>
	...
	<form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
		<div class="main">
			...
			<div class="form-row">
				<label>
					<span>آدرس سایت فیلتر شده را وارد کنید:</span>
					<input type="text" name="<?php echo htmlspecialchars($GLOBALS['_config']['url_var_name']) ?>" value="<?php echo isset($_GET[$GLOBALS['_config']['url_var_name']]) ? htmlspecialchars(decode_url($_GET[$GLOBALS['_config']['url_var_name']])) : (isset($_GET['__iv']) ? htmlspecialchars($_GET['__iv']) : ''); ?>" placeholder="آدرس سایت فیلتر را که میخواهید وارد آن شوید را در اینجا بنویسید" required="required"/>
				</label>
			</div>
			<div class="form-row">
				<button class="button-submit" type="submit">رفع فیلتر دولت</button>
				<label class="button-cancel" for="proxopttogl">تنظیمات</label>
			</div>
<?php
...
<input type="checkbox" id="proxopttogl"/>
		<div id="proxoptmenu" class="main">
			...
			<div class="prx-opt-menu">
				<li id="newWin" class="option" style="display: none;"><label><input type="checkbox"/>نمایش پنجره ای</label></li>
<?php
